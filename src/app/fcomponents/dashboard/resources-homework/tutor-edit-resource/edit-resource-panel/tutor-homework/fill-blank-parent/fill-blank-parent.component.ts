...
@Component({
  selector: 'app-fill-blank-parent',
  templateUrl: './fill-blank-parent.component.html',
  styleUrls: ['./fill-blank-parent.component.css']
})
export class FillBlankParentComponent implements OnInit {
      ...
      msg => {
        if(Object.keys(msg).length != 0) {
          ...
          if (msg['op'] == "request2Save") {
            ...
            this.resourceCtrlService.sendSavedQuestionArray("blank", this.questionArray);
          }
        }
      }
    );
    ...
    if (this.questionArray.length === 0) {
      ...
      if (this.questionArray.length < this.questionAllowedNum) {
        if (this.questionArray[this.questionArray.length - 1].questionSaveStatus == "init") {
          ...
          let newQuizObj = this.getQuestionObj("blank", this.questionArray.length + 1, {});
