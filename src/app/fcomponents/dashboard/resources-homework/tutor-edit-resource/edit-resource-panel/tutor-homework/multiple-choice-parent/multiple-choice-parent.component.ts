...
@Component({
  selector: 'app-multiple-choice-parent',
  templateUrl: './multiple-choice-parent.component.html',
  styleUrls: ['./multiple-choice-parent.component.css'],
  //directives: [MultipleChoiceComponent]
})
export class MultipleChoiceParentComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.signalPropertySubscription = this.resourceCtrlService.signalProperty.subscribe(
      msg => {
        if(Object.keys(msg).length != 0) {
          console.log("<<MultipleChoices Parent>> receives signal message:");
          console.log(msg);
          if (msg['op'] == "request2Save") {
            console.log("<<MultipleChoices Parent>> receives signal to send back question array");
            this.resourceCtrlService.sendSavedQuestionArray("multi", this.questionArray);
          }
        }
      }
    );
    ...
    this.questionArraySubscription = this.resourceCtrlService.questionArray.subscribe(
      msg => {
        if(Object.keys(msg).length != 0) {
          console.log("<<MultipleChoices Parent>> receives questionArray message:");
          console.log(msg);
          let op = msg['op'];
          if (op == "init" && this.accessType == "modify") {
            console.log("<<MultipleChoices Parent>> receives `init` operation command....");
            this.questionArray = msg['value'];
          } else if (op == "edit") {
            console.log("<<MultipleChoices Parent>> receives `edit` operation command....");
            console.log(msg['value']);
            this.questionArray = msg['value'];
          } else if(op == "save") {
            console.log("<<MultipleChoices Parent>> receives `save` operation command.... (useless)");
          }
        }
      }
    );
    ...
    if (this.questionArray.length === 0) {
      ...
      if (this.questionArray.length < this.questionAllowedNum) {
        if (this.questionArray[this.questionArray.length - 1].questionSaveStatus == "init") {
          ...
          let newQuizObj = this.getQuestionObj("multi", this.questionArray.length + 1, {});
